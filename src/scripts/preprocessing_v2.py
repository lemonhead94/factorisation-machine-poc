...
def build_adj_mx(n_feat: np.int32, data: npt.NDArray[np.int32]) -> sp.dok_matrix:
...
def build_heterogeneous_adj_mx(
    dims: npt.NDArray[np.int32],
    data_dish_ingredient: npt.NDArray[np.int32],
    data_user_ingredient: npt.NDArray[np.int32],
) -> sp.dok_matrix:
    num_users = dims[0]
    num_dishes = dims[1]
    num_ingredients = dims[2]
    ...
    heterogeneous_adj_mx = sp.hstack(
        [user_dish_mat, dish_ingredient_mat, user_ingredient_mat]
    )
...
def preprocessing(args: Namespace) -> None:
    ...
    heterogeneous_adj_mx = build_heterogeneous_adj_mx(
        dims=np.array(
            [args.num_users, args.num_dishes, args.num_ingredients], dtype=np.int32
        ),
        data_dish_ingredient=train_dish_ingredient,
        data_user_ingredient=train_user_ingredient,
    )
    sp.save_npz(
        f"{args.output_path}/heterogeneous_adjacency_matrix.npz", heterogeneous_adj_mx
    )
