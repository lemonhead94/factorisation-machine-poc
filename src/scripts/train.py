...
if __name__ == "__main__":
    ...
    user_ingredient_train_loader = DataLoader(user_ingredient_train, batch_size=batch_size, shuffle=True)
    dish_ingredient_train_loader = DataLoader(dish_ingredient_train, batch_size=batch_size, shuffle=True)
    user_ingredient_test_loader = DataLoader(user_ingredient_test, batch_size=batch_size, shuffle=False)
    dish_ingredient_test_loader = DataLoader(dish_ingredient_test, batch_size=batch_size, shuffle=False)

    # Initialize the model and optimizer
    model = FactorizationMachineModel_withGCN(field_dims=[...], embed_dim=...)  # Replace with appropriate field_dims and embed_dim
    ...
    for epoch in range(num_epochs):
        train_loss_user_ingredient = train(model, user_ingredient_train_loader, gat_optimizer, criterion, device)
        train_loss_dish_ingredient = train(model, dish_ingredient_train_loader, gat_optimizer, criterion, device)
